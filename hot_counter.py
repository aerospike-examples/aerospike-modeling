...
try:
    for i in range(1, 6, 1):
        k = (
            namespace,
            set,
            "cf296d9a-0b77-4dd0-8d2b-91e59a6f02d2|adx123" + "|2019-03-05||" + str(i),
        )
        shards.append(k)
        try:
            client.remove(k)
...
try:
    print(
        "One way to identify this key is now an indicator for a sharded",
...
for i in range(0, options.num, 1):
    try:
        ...
        if val < options.cap:
            ...
            client.increment(
                shard_key, "c", 1, {}, {"ttl": aerospike.TTL_DONT_UPDATE}
            )  # aka ttl -2
